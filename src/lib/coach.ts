...
import { GROWTH_PLAYBOOK } from "./growthKnowledge";
...
- フォロワー: 日次手入力+折れ線グラフ+連続記録N日表示。未入力日はアナリティクス概要CSVの
  増減から自動補完(推定表示)
- 週次: 週ごとカードの数字サマリー(リプ数/純増/インプの行とリプあり週vsリプ0週の平均純増比較付き)・
  30日ペース換算・学び一覧・時間簿・
...
- プロフィール画面(設定から): 名前/bio(160字)の保存履歴・bioの3項目AI診断+改善版1案・
  変更日はフォロワーグラフに縦点線表示・固定ポストの選択(実測インプ上位3件の候補付き)と
  変更履歴(前後7日のプロフ→フォロー転換率比較、グラフに紫点線)
